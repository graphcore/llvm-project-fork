...
//===-- st16f_misaligned_test.c - Test store builtins----------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file tests __st16f_misaligned for the compiler_rt library.
//
//===----------------------------------------------------------------------===//
...
int check(unsigned offset, unsigned first, unsigned second) {
  ...
  // Write data to the offset (0.000757 has hex representation of 0x1234).
  __st16f_misaligned((unsigned) &mem[offset], 0.000757);
